    ...
    var body: some View {
        ZStack {
            Button(action: {
                ...
                Image(uiImage: Constants.Image.petmovie!)
                    .resizable()
...
struct PetMovieVideoView: View {
    var path = Bundle.main.path(forResource: "", ofType: "")
    var body:  some  View {
        ZStack {
            ZStack {
        VideoPlayer (
            player: AVPlayer(url: URL(fileURLWithPath: Bundle.main.path(forResource: "petmovie", ofType: "mp4")!))
         )
        }
            ZStack(alignment: .bottomLeading) {
                ...
                VStack (spacing: 35) {
                    Spacer()
                    if UserDefaults.standard.string(forKey: "industry") == "Movies" {
                  VideoAnimation()
                    } else {
                        Spacer()
