    ...
    var body: some View {
        ZStack {
            Button(action: {
                ...
                Image(uiImage: Constants.Image.extremeAva!)
                    .resizable()
...
struct ExtremeVideoView: View {
    var path = Bundle.main.path(forResource: "", ofType: "")
    var body:  some  View {
        ZStack {
            ZStack {
        VideoPlayer (
            player: AVPlayer(url: URL(fileURLWithPath: Bundle.main.path(forResource: "extreme", ofType: "mp4")!))
         )
        }
