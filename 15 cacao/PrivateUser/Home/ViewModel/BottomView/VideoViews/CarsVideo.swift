    ...
    var body: some View {
        ZStack {
            Button(action: {
                ...
                Image(uiImage: Constants.Image.car!)
                    .resizable()
...
struct CarsVideoView: View {
    var path = Bundle.main.path(forResource: "", ofType: "")
    var body:  some  View {
        ZStack {
            ZStack {
        VideoPlayer (
            player: AVPlayer(url: URL(fileURLWithPath: Bundle.main.path(forResource: "cars", ofType: "mp4")!))
         )
        }
            ZStack(alignment: .bottomLeading) {
                ...
                VStack (spacing: 35) {
                    Spacer()
                    if UserDefaults.standard.string(forKey: "industry") == "Cars" {
                  VideoAnimation()
                    } else {
                        Spacer()
