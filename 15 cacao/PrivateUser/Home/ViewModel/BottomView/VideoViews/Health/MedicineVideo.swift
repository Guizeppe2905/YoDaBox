    ...
    var body: some View {
        ZStack {
            Button(action: {
                ...
                Image(uiImage: Constants.Image.a6!)
                    .resizable()
...
struct MedicineVideoView: View {
    var path = Bundle.main.path(forResource: "", ofType: "")
    var body:  some  View {
        ZStack {
            ZStack {
        VideoPlayer (
            player: AVPlayer(url: URL(fileURLWithPath: Bundle.main.path(forResource: "medicine", ofType: "mp4")!))
         )
        }
