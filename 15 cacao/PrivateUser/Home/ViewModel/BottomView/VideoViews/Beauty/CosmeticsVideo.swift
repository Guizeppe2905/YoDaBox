    ...
    var body: some View {
        ZStack {
            Button(action: {
                ...
                Image(uiImage: Constants.Image.a9!)
                    .resizable()
...
struct CosmeticsVideoView: View {
    var path = Bundle.main.path(forResource: "", ofType: "")
    var body:  some  View {
        ZStack {
            ZStack {
        VideoPlayer (
            player: AVPlayer(url: URL(fileURLWithPath: Bundle.main.path(forResource: "makeup", ofType: "mp4")!))
         )
        }
            ZStack(alignment: .bottomLeading) {
                ...
                VStack (spacing: 35) {
                    Spacer()
                    if UserDefaults.standard.string(forKey: "industry") == "Fashion" || UserDefaults.standard.string(forKey: "industry") == "Beauty" {
                  VideoAnimation()
                    } else {
                        Spacer()
