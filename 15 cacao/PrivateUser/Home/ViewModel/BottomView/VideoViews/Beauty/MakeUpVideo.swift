    ...
    var body: some View {
        ZStack {
            Button(action: {
                ...
                Image(uiImage: Constants.Image.a3!)
                    .resizable()
...
struct MakeUpVideoView: View {
    ...
    var body:  some  View {
        ZStack {
            ZStack {
        VideoPlayer (
            player: AVPlayer(url: URL(fileURLWithPath: Bundle.main.path(forResource: "makeup2", ofType: "mp4")!))
         )
        }
