...
//  15 cacao
//
//  Created by Мария Хатунцева on 21.06.2022.
//

import SwiftUI
import AVKit

struct ButtonOriental: View {
    ...
    var body: some View {
        ZStack {
            Button(action: {
                ...
                Image(uiImage: Constants.Image.orientalAva!)
                    .resizable()
...
struct OrientalVideoView: View {
    var path = Bundle.main.path(forResource: "", ofType: "")
    var body:  some  View {
        ZStack {
            ZStack {
        VideoPlayer (
            player: AVPlayer(url: URL(fileURLWithPath: Bundle.main.path(forResource: "oriental", ofType: "mov")!))
         )
        }
